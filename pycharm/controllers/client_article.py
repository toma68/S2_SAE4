...
def client_article_show():
    ...
    recuperation_article = '''SELECT DISTINCT vetement.id_vetement,libelle_vetement,libelle_marque,prix_vetement, envie.id_users FROM vetement
    ...
    tailles_achat = '''SELECT * FROM taille 
        join est_dispo ed on ed.id_taille = taille.id_taille'''
    mycursor.execute(tailles_achat)
    tailles_achat = mycursor.fetchall()
    ...
    recuperation_envie = '''SELECT id_vetement FROM envie
       WHERE id_users = %s'''
    tuple_recup_envie = (session['user_id'])
    mycursor.execute(recuperation_envie, tuple_recup_envie)
    envies = mycursor.fetchall()
    envies_return = []
    for key in envies:
        envies_return.append(key['id_vetement'])


    return render_template('client/boutique/show_articles.html', articles=articles_return, articlesPanier=articles_panier,
                           prix_total=prix_total, itemsFiltreType=types_articles, tailles=tailles, sexes = sexes,tailles_achat=tailles_achat, envies = envies_return)
