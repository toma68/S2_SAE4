...
def show_article():
    ...
    recuperation_article = '''SELECT DISTINCT vetement.id_vetement,libelle_vetement,libelle_marque,prix_vetement, sum(stock) as stock_global FROM vetement
    ...
    left join est_dispo on est_dispo.id_vetement=vetement.id_vetement
    group by vetement.id_vetement, libelle_vetement, libelle_marque, prix_vetement
    '''
...
def valid_edit_article():
    ...
    message = u'article Modifié:  nom:'+nom + ', type_article:' + type + ', prix:' + prix + ', taille(s):'+  tailles + ' styles :' + styles + ' marque(s) :' + marques + ' matière(s) : ' + matieres + ' merci et bonne journée ! '
    flash(message)
    return redirect(url_for('admin_article.show_article'))
