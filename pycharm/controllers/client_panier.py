...
@client_panier.route('/client/panier/modif/')      # remplace /client
def client_panier_addone():                                 # remplace client_index
    mycursor = get_db().cursor()
    quantite = request.args['quantite']
    taille = request.args['taille']
    id_article = request.args['id']
    tuple_select = (id_article,taille)
    ...
    if (int(quantite) <= quantite_dispo):
        requete = '''UPDATE panier SET quantite = %s          
        where id_users = %s and id_vetement = %s and id_taille = %s;'''
        tuple_update = (quantite,session['user_id'], id_article,taille)
        print(tuple_update)
        mycursor.execute(requete,tuple_update)
        get_db().commit()
        flash('Quantité passée à ' + quantite)
    else:
        flash('Il n\'y a que ' + str(quantite_dispo) + ' articles disponibles, il n\'est donc pas possible d\'en rajouter ' + quantite + ' !' )
...
def client_panier_commander():
    ...
    if (False not in dispo):
        ...
        recup_commande = '''SELECT * FROM commande ORDER BY id_commande DESC LIMIT 0,1'''
...
@client_panier.route('/client/wishlist/show')      # remplace /client
def client_wishlist_show():
    mycursor = get_db().cursor()
    ...
    for key in articles_requete:
        if (key['id_vetement'] not in articles_id):
            ...
            articles_return[i - 1]['libelle_marque'] += ' x ' + key['libelle_marque']
    return render_template('client/boutique/wishlist.html',articles = articles_return)



@client_panier.route('/client/wishlist/delete/<int:id>&<int:choix>')      # remplace /client
def client_wishlist_delete(id,choix):
    mycursor = get_db().cursor()
    requete ='''DELETE FROM envie 
    where id_users = %s
    and id_vetement = %s'''
    tuple_delete=(session['user_id'],id)
    mycursor.execute(requete,tuple_delete)
    get_db().commit()
    if (choix == 0):
        return redirect('/client/article/show')
    return redirect('/client/wishlist/show')


@client_panier.route('/client/wishlist/add/<int:id>')      # remplace /client
def client_wishlist_add(id):
    mycursor = get_db().cursor()
    requete ='''INSERT INTO envie VALUES(%s,%s,DATE(NOW()))'''
    tuple_insert=(session['user_id'],id)
